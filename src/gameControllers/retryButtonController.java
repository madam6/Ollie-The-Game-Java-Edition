...
public class retryButtonController implements MouseListener, ActionListener {
    Button retryButtom;
    Timer clickTimer;
    SoundClip buttonSound, clickSound;
    Game game;
    GameView gameView;
    public retryButtonController(Button rb, Game game, GameView gv) {
        gameView = gv;
        this.game = game;
        this.retryButtom = rb;
        try {
    ...
    public void actionPerformed(ActionEvent e) {
        if(e.getSource()==clickTimer) {
            clickTimer.stop();
            gameView.retry.setVisible(false);
            gameView.back.setVisible(false);
            gameView.remove(gameView.retry);
            gameView.remove(gameView.back);
            game.retryLevel();
        }
    }
}
